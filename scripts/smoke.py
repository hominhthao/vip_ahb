#!/usr/bin/env python3
"""Compile and run the standalone transaction smoke test with VCS."""
...
def main():
    ...
    build_dir = project_root / "work" / "transaction_smoke"
    ...
    compile_command = [
        ...
        str(project_root / "vip" / "example" / "tb" /
            "fpt_ahb_transaction_smoke_top.sv"),
        "-top", "fpt_ahb_transaction_smoke_top",
    ...
    if "PASS: transaction smoke test (" not in run_log:
        print("FAIL: smoke test completion marker missing", flush=True)
        return 1
    print("PASS: transaction smoke compile and simulation (seed=1).", flush=True)
    return 0
